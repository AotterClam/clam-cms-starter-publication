...
export function buildCmsConfig(env: Env, auth: Auth): CmsConfig {
  return {
    ...
    siteDefaults: {
      ...
      // install time (ADR-0016). JSON.parse keeps this file TS-valid
      // pre-substitution so contributors can `pnpm typecheck` the
      // starter directly; the runtime cost is one tiny parse at worker
      // cold-start.
