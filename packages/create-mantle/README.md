...
```bash
npx https://github.com/AotterClam/mantle/releases/download/v0.0.8-alpha.1/aotterclam-create-mantle-0.0.8-alpha.1.tgz \
  <archetype> \
  --project-name <name> \
  --brand "<brand>" \
...
1. Fetches `sources.json` from `AotterClam/mantle-starters` at the requested ref (default `main`) — resolves archetype → starter path + (optional) theme overlay. Falls back to a bundled stale snapshot if GH is unreachable.
