...
const clamCmsVersion = args["mantle-version"] ?? readPackageVersion();
...
rewriteJson("package.json", (json) => ({
  ...json,
  dependencies: rewriteClamCmsDeps(json.dependencies ?? {}, clamCmsVersion),
}));
rewriteJson("tsconfig.json", (json) => ({ ...json, extends: "./tsconfig.base.json" }));

console.log(`Configured ${projectName}`);
console.log(`Locales: ${locales.join(", ")}`);
console.log(`mantle packages: ${clamCmsVersion}`);
